...
function getCustomerCollection(docId) {
    console.log("called");
    db.collection("Customer").doc(docId).collection("Customer's Details").get()
        .then((snapshot) => {
            snapshot.docs.forEach(doc => {
                console.log(doc.data()["Custumer's Name"])
                console.log(doc.data()["Custumer's Address"])
                console.log(doc.data()["Custumer's Mobile"])
                console.log(doc.id);
                renderItems(doc)
            })
        })
...
function renderItems(doc) {
    ...
    cMobile.textContent = doc.data()["Custumer's Mobile"];
    ...
    document.getElementById("orders").onclick = function() { getCustomersOrders() };
}
...
function getCustomersOrders() {
    db.collection("orders").get().then((
        snapshot) => {
        snapshot.docs.forEach(doc => {
            db.collection("order_status").get().then(snap => {
