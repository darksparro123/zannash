...
function getShopsFromFirestore() {
    ...
    db.collection("shops").where("admin permission", "==", false).get()
        .then((snapshot) => {
...
function getShopOwnersDetails(shopId) {
    db.collection("shops").doc(shopId).collection("Owner Details").get()
        .then(snapshot => {
            snapshot.docs.forEach(doc => {
...
function getShopBankDetails(shopId) {
    db.collection("shops").doc(shopId).collection("Bank Details").get()
        .then(snapshot => {
            snapshot.docs.forEach(doc => {
...
function renderShopDetails(doc) {
    ...
    registrationCertificate.setAttribute("src", doc.data()["Shop Registration Certificate"])
    ...
    document.getElementById("update-button").onclick = function() { givePermissionsForShops(doc.data()["Email"]) };
}
